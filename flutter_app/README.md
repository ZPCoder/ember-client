...
- WebSocket 联机大厅与联机战场：创建/加入 1v1 房间、提交合法牌组、自动起手、服务器权威出牌/攻击/回合与生命值同步
...
本地 Dart 服务只用于连接与房间 UI 测试；生产站点的 `/api/pvp` Worker 才是正式联机入口，负责牌组校验、隐藏手牌/奥秘、规则 reducer、回合锁、超时结束和断线快照同步。Flutter 联机战场只渲染服务器脱敏快照，不在客户端复制一套可被篡改的规则引擎。
